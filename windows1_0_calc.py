...
def start_calc():
    ...
    Calc = Tk()
    Calc.title('Calculator.exe')
    label = Label(Calc, text='In memory: %s' % (memory))
    string = Entry(Calc)
    ...
    new_item5.add_command(label='Close', command=closeCalc)
    new_item6 = Menu(menu, tearoff=0)
    new_item6.add_command(label='Copy', command=add)
    new_item6.add_command(label='Paste', command=MR)
    menu.add_cascade(label='Menu', menu=new_item5)
    menu.add_cascade(label='Edit', menu=new_item6)
